// Import the main-thread decoder directly; the package root also imports its
// browser Web Worker wrapper, which is unavailable in Cloudflare Workers.
// @ts-expect-error The package only publishes declarations for its root export.
import MPEGDecoder from "../node_modules/mpg123-decoder/src/MPEGDecoder.js";
import mpg123Wasm from "./mpg123.wasm";
...
const EDGE_AUDIO_CONTENT_TYPE = "audio/mpeg";
...
async function* ttsStream(
  ...
  ws.addEventListener("message", (event) => {
    ...
    if (typeof data === "string") {
      ...
      if (buffer.length >= 2) {
        ...
        if (headers["Path"] === "audio" && headers["Content-Type"]?.toLowerCase() === EDGE_AUDIO_CONTENT_TYPE && audioData.length > 0) {
          messageQueue.push({ type: "audio", data: audioData });
        } else if (headers["Path"] === "audio" && audioData.length > 0) {
          console.warn("Ignoring audio chunk with unexpected content type:", headers["Content-Type"]);
        }
      }
    }
    notify();
  ...
  // Send speech config
  ws.send(
    `X-Timestamp:${dateToString()}\r\nContent-Type:application/json; charset=utf-8\r\nPath:speech.config\r\n\r\n{"context":{"synthesis":{"audio":{"metadataoptions":{"sentenceBoundaryEnabled":"false","wordBoundaryEnabled":"true"},"outputFormat":"${EDGE_AUDIO_FORMAT}"}}}}\r\n`
  );
...
async function mp3ToWav(chunks: Uint8Array[]): Promise<Uint8Array> {
  const mp3Length = chunks.reduce((total, chunk) => total + chunk.byteLength, 0);
  const mp3 = new Uint8Array(mp3Length);
  let mp3Offset = 0;
  for (const chunk of chunks) {
    mp3.set(chunk, mp3Offset);
    mp3Offset += chunk.byteLength;
  }

  MPEGDecoder.module = mpg123Wasm;
  const decoder = new MPEGDecoder();
  await decoder.ready;
  const decoded = decoder.decode(mp3);
  decoder.free();
  if (decoded.errors.length > 0 || decoded.samplesDecoded === 0) {
    throw new Error(decoded.errors[0]?.message || "Failed to decode MP3 audio");
  }

  const inputChannels = decoded.channelData.length;
  const channels = 1;
  const dataLength = decoded.samplesDecoded * channels * WAV_BITS_PER_SAMPLE / 8;
  ...
  view.setUint32(28, decoded.sampleRate * blockAlign, true);
  ...
  let offset = 44;
  for (let sample = 0; sample < decoded.samplesDecoded; sample++) {
    let mixed = 0;
    for (let channel = 0; channel < inputChannels; channel++) mixed += decoded.channelData[channel][sample];
    const value = Math.max(-1, Math.min(1, mixed / inputChannels));
...
async function handleSpeech(request: Request, env: Env): Promise<Response> {
  ...
  if (body.response_format && body.response_format.toLowerCase() !== "wav") {
    return errorResponse("'response_format' must be 'wav'", 400);
  }
  ...
  try {
    for await (const audioData of ttsStream(input, voice, rate)) chunks.push(audioData);
  } catch (e: any) {
    console.error("TTS error:", e);
    return errorResponse(e.message || "Speech synthesis failed", 502);
  }

  let wav: Uint8Array;
  try {
    wav = await mp3ToWav(chunks);
  } catch (e: any) {
    console.error("Audio conversion error:", e);
    return errorResponse(e.message || "Audio conversion failed", 502);
  }
  ...
  server.addEventListener("message", (event) => {
    const handle = async () => {
      ...
      try {
        const chunks: Uint8Array[] = [];
        for await (const audioData of ttsStream(input, voice, rate)) chunks.push(audioData);
        server.send(await mp3ToWav(chunks));
